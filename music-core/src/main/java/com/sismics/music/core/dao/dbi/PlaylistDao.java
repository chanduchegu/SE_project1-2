...
public class PlaylistDao extends BaseDao<PlaylistDto, PlaylistCriteria> {
    ...
    protected QueryParam getQueryParam(PlaylistCriteria criteria, FilterCriteria filterCriteria) {
        ...
        //StringBuilder sb= new StringBuilder("SELECT ID, USER_ID, NAME FROM T_PLAYLIST WHERE USER_ID = '[USER_ID]' OR isPrivate = false");
        ...
        if(criteria.getIsPrivate()!=null){
            ...
            if (! criteria.getIsPrivate()) {
                ...
                criteriaList.add("p.isPrivate = false");
            }
        }
...
//        handle.createStatement("alter table T_PLAYLIST\n" +
//                "add isPrivate boolean").execute();
        handle.createStatement("insert into " +
                "  t_playlist(id, user_id, name ,isPrivate)" +
                "  values(:id, :userId, :name,:isPrivate)")
                .bind("id", playlist.getId())
                .bind("userId", playlist.getUserId())
                .bind("name", playlist.getName())
                .bind("isPrivate", playlist.getIsPrivate())
                .execute();
